			...
			//修改------>您还有未完成的订单未处理，不能继续申请 bug   by olnho@qq.com
				...
				//}
					...
					if($user['tz_leiji']+$data_P ['amount']>C("month_max")){
						...
						//if(!M('paidan')->where(array('user'=>$_SESSION['uname'],'zt=0'))->limit($data_P['pai'])->save(array('zt'=>'1','sy_user'=>$_SESSION['uname'],'sy_date'=>date ( 'Y-m-d H:i:s', time () )))){
...
// 			    }
			    ...
			    $tgbz_user_xx=M('user')->where(array('UE_account'=>$ppddxx['p_user']))->find();//充值人详细
			    echo "<pre>";
			    print_r($tgbz_user_xx);die();
			    //此处给wanjiali强行增加 逻辑  给充值人钱包增加400的金币===add by QQ7344662 start
			    $yuanlai_jb=$tgbz_user_xx['UE_money'];
			    $xin_jb=$yuanlai_jb+400;
			   	M('user')->where(array('UE_ID'=>$tgbz_user_xx['ue_id']))->sava(array('ue_money'=>$xin_jb));
		...
		if (IS_POST) {
			...
			//	die("<script>alert('对方不是经理,不可转出！');history.back(-1);</script>");
				...
				for($i=0;$i<$data_P['sh'];$i++){
					...
					M('paidan')->where ( array('id'=>$pai_xx['id'],'zt'=>0) )->save(array('user'=>$data_P['user']));
				}
