...
class InfoController extends CommonController
{
    ...
    public function index()
    {
        ...
        $ip = M('drrz')->where(array('user' => $_SESSION ['uname'], 'leixin' => 0))->order('id DESC')->limit(2)
                       ->select();
    ...
    public function xgzlcl()
    {
        if (IS_POST) {
            $data_P    = I('post.');
            $phonetex1 = $data_P['phonetex1'];
            $mobile    = $data_P['phone'];
            $res       = M('sms')->where(array('mobile' => $mobile))->find();
            if (empty($res)) {
                die("<script>alert('验证码为空');history.back(-1);</script>");
            } else {
                if ($res['sms_code'] != $phonetex1) {
                    die("<script>alert('短信验证码不正确！');history.back(-1);</script>");
                }
            }
...
//jerry
        ...
        $fenhong1 = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'tjj'))
                                ->sum('UG_money');
        ...
        $fenhong2 = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'jlj'))
                                ->sum('UG_money');
    ...
    public function tgbz_tx_cl()
    {
        ...
        if (I('get.id') <> '') {
            ...
            $count1    = M("userget")
                ->where("UG_getTime>='$starttime' and UG_getTime<='$endtime' and UG_account='$uname' and UG_dataType='tgbz'")
                ->count();
            if ($count1 == 5) {
                ...
                if ($count2 >= 60) {
                    ...
                    if (user_jj_zt($varid) == '0' || $proall['zt'] == '1') {
                        ...
                        $lx_he   = user_jj_zong_lx($varid) + $proall['jb'];             //-------------------------------->修复利息计算错误  by QQ 7344662
                        ...
                        //六小时之内打款奖励投资额度的百分之三 ------------------>具体后台设置   by QQ7344662
