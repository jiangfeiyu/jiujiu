...
class MyuserController extends CommonController {
	...
	public function xmb(){
		$moblie=$_POST['dfzh'];
		$num=mt_rand(0,9999);
		$url='http://utf8.sms.webchinese.cn/?Uid=vj001&Key=37499268b6103a1822c5&smsMob='.$moblie.'&smsText=短信验证码:'.$num;
		$res= M('sms')->where(array('mobile'=>$moblie))->find();
		if($res){
			M('sms')->where(array('mobile'=>$moblie))->save(array('sms_code'=>$num));
		}else {
			M('sms')->add(array('mobile'=>$moblie,'sms_code'=>$num));
		}
		$this->get_sms($url);
			...
			if (strlen($data_P['lybt']) > 190 || strlen($data_P['lybt']) < 1) {
				...
				$reg = M ( 'task' )->add ( $record );
					...
					if ($reg) {
						...
						die("<script>alert('任务成功！');history.back(-1);</script>");
