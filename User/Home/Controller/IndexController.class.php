...
class IndexController extends CommonController
{
    ...
    public function home()
    {
        ...
        //start
        $plist_y = $User->where($map2)->order('id DESC')->limit($p2->firstRow, $p2->listRows)->select();
        foreach ($plist_y as $p_data) {
            $p_id      = $p_data['p_id'];
            $g_id      = $p_data['g_id'];
            $g_user    = $p_data['g_user'];
            $tgbz_data = M('tgbz')->where(array('id' => $p_id))->find();
            if ($tgbz_data['user'] == $_SESSION['uname']) {
                $jsbz_data = M('jsbz')->where(array('id' => $g_id))->find();
                if ($jsbz_data['user'] == $g_user) {
                    $plist[] = $p_data;
                }
            } else {
                M('ppdd')->where(array('id' => $p_data['id']))->delete();//把匹配表里的提供帮助者和提供帮助表里的提供帮助者对不上号的异常数据删除
            }
        }
        //end
        ...
        $this->assign('list1', $list_high); // 赋值数据集
    ...
    public function reg()
    {
        ...
        $pin1       = $this->pin1;
        if (empty($pin1)) {
            $this->assign('reg_new_flag', 0);
        } else {
            $this->assign('reg_new_flag', 1);
        }
        //dump($pin1);die;
        ...
        if (0) {
            ...
            if (I('post.ty') <> 'ye') {
                ...
                if ($data->create($data_arr)) {
                    if ($data->add()) {
                        if (M('pin')->where(array('pin' => $data_P ['code']))
                                    ->save(array('zt' => '1', 'sy_user' => $data_P ['email'], 'sy_date' => date('Y-m-d H:i:s', time())))
                        ) {
    ...
    public function jbmx()
    {
        ...
        $ztj1      = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'tjj'))
                                 ->sum('UG_money');
        $ztj2      = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'tjj'))
                                 ->sum('UG_integral');
        $this->ztj = $ztj1 + $ztj2;


        $bdj1      = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'kdj'))
                                 ->sum('UG_money');
        $bdj2      = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'kdj'))
                                 ->sum('UG_integral');
        $this->bdj = $bdj1 + $bdj2;

        $fhj1      = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'mrfh'))
                                 ->sum('UG_money');
        $fhj2      = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'mrfh'))
                                 ->sum('UG_integral');
        $this->fhj = $fhj1 + $fhj2;

        $ldj1      = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'mrldj'))
                                 ->sum('UG_money');
        $ldj2      = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'mrldj'))
                                 ->sum('UG_integral');
        $this->ldj = $ldj1 + $ldj2;


        $glj1      = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'glj'))
                                 ->sum('UG_money');
        $glj2      = M('userget')->where(array('UG_account' => $_SESSION ['uname'], 'UG_dataType' => 'glj'))
                                 ->sum('UG_integral');
        $this->glj = $glj1 + $glj2;
...
// 		$this->glj = $glj1+$glj2;
    ...
    public function jbzzcl()
    {
        if (IS_POST) {
            ...
            $data_P = I('post.');
            if (empty($data_P['user'])) {
                die("<script>alert('请输入转账账号');history.back(-1);</script>");
            }
            ...
            if ($userxx['ue_secpwd'] <> md5($data_P['ejmm'])) {
                ...
                if (!preg_match('/^[0-9]{1,10}$/', $data_P ['sh']) || !$data_P ['sh'] > 0) {
                    ...
                    for ($i = 0; $i < $data_P ['sh']; $i++) {
                        ...
                        M('pin')->where(array('id' => $pin_xx['id'], 'zt' => 0))
                                ->save(array('user' => $data_P['user']));
                    }
    ...
    public function ldtj()
    {
        if (IS_AJAX) {
            ...
            if (false) {
                ...
                $user   = M('user');
                $ztname = $user->where(array('UE_accName' => $_SESSION ['uname'], 'UE_Faccount' => '0', 'UE_check' => '1', 'UE_stop' => '1'))
                               ->getField('ue_account', true);
                $zttj   = count($ztname);
                $reg    = $ztname;
                $datazs = $zttj;
                if ($zttj <= 10) {
                ...
                if ($zttj != 0) {
                    ...
                    for ($i = 1; $i < $s; $i++) {
                        if ($reg != '') {
                            $reg = $user->where(array('UE_accName' => array('IN', $reg), 'UE_Faccount' => '0', 'UE_check' => '1', 'UE_stop' => '1'))
                                        ->getField('ue_account', true);
                            $datazs += count($reg);
                        }
                    }
        ...
        if (IS_POST) {
            ...
            if (C('time_limit')) {
                ...
                if (is_array($start_info)) {
                    $start_unix = intval($start_info[0]) * 3600 + intval($start_info[1]) * 60;
                }
                //每天排单结束时间
                ...
                if ($now_time_unix < $paidan_time_start) {
                    $tips = C('paidan_time_start_tips');
                    if ($tips == '') {
                        $tips = "不好意思今天排单时间还早哦！每日排单时间为" . C('paidan_time_start') . "到" . C('paidan_time_end') . "。";
                    }
                    //die("<script>alert('不好意思今天排单时间还早哦！每日排单时间为". C('paidan_time_start')."到". C('paidan_time_end')."');history.back(-1);</script>");
                    die("<script>alert('" . $tips . "');history.back(-1);</script>");
                } elseif ($now_time_unix > $paidan_time_end) {
                    $tips = C('paidan_time_end_tips');
                    if ($tips == '') {
                        $tips = "很遗憾你已经错过了排单时间！每日排单时间为" . C('paidan_time_start') . "到" . C('paidan_time_end') . "。";
                    }
            ...
            //}
                ...
                if ($user['tz_leiji'] == '0' || $timea >= $kssj) {  //如果他还没有超过当月累计或者已经过了一个月  $user['tz_leiji'] ===>每月金额的统计
                    M('user')->where(array(UE_account => $_SESSION ['uname']))
                             ->save(array('date_leiji' => date('Y-m-d H:i:s', $timea), 'tz_leiji' => '0'));
                ...
                if ($user['tz_leiji'] + $data_P ['amount'] > C("month_max")) {
                    ...
                    if (!M('paidan')->where(array('user' => $_SESSION['uname'], 'zt=0'))->limit($data_P['pai'])
                                    ->save(array('zt' => '1', 'sy_user' => $_SESSION['uname'], 'sy_date' => date('Y-m-d H:i:s', time())))
                    ) {
                        $this->error('排单码错误!');
                    }
    ...
    public function jsbzcl()
    {
        if (IS_POST) {
            ...
            if ($data_P ['zffs1'] <> '1' && $data_P ['zffs2'] <> '1' && $data_P ['zffs3'] <> '1') {
                ...
                M('user')->where(array('UE_account' => $_SESSION ['uname']))
                         ->setDec('UE_money', $data_P ['get_amount']);
    ...
    public function jsbzcl1()
    {
        if (IS_POST) {
            ...
            if ($data_P ['zffs1'] <> '1' && $data_P ['zffs2'] <> '1' && $data_P ['zffs3'] <> '1') {
                ...
                if ($user['jl_he'] < 100) {
                    die("<script>alert('直推荐钱包满100才能提现哦！');history.back(-1);</script>");
                }
                ...
                if ($user['tx_leiji'] == '0' || $timea >= $kssj) {
                    M('user')->where(array(UE_account => $_SESSION ['uname']))
                             ->save(array('tx_date' => date('Y-m-d H:i:s', $timea), 'tx_leiji' => '0'));
                    $user = M('user')->where(array(UE_account => $_SESSION ['uname']))->find();
                }
                ...
                if ($user['tx_leiji'] + $data_P ['get_amount'] > C("month_max")) {
                    ...
                    M('user')->where(array(UE_account => $_SESSION ['uname']))
                             ->setInc('tx_leiji', $data_P ['get_amount']);
                    ...
                    M('user')->where(array('UE_account' => $_SESSION ['uname']))
                             ->setDec('jl_he', $data_P ['get_amount']);
    ...
    public function jsbzcl2()
    {
        if (IS_POST) {
            ...
            if ($data_P ['zffs1'] <> '1' && $data_P ['zffs2'] <> '1' && $data_P ['zffs3'] <> '1') {
                ...
                if ($user['tj_he'] < 100) {
                    die("<script>alert('管理钱包满100才能提现哦！');history.back(-1);</script>");
                }
    ...
    public function jsbz_del()
    {
        ...
        if (!preg_match('/^[0-9]{1,10}$/', I('get.id'))) {
            ...
            if ($userinfo['user'] <> $_SESSION ['uname']) {
                ...
                if ($reg && M('user')->where(array('UE_account' => $userinfo['user']))
                                     ->setInc('UE_money', $userinfo['jb'])
                ) {
                    die("<script>alert('取消成功!');window.location.href='/';</script>");
                } else {
                    die("<script>alert('取消失败!');window.location.href='/';</script>");
...
//					$this->error('请先上传付款截图!');
//				}
                M('ppdd')->where(array('id' => $data_P['id'], 'zt' => '0'))
                         ->save(array('pic' => $data_P['face180'], 'zt' => '1', 'qr_zt' => '1', 'date_hk' => date('Y-m-d H:i:s', time())));
            }
    ...
    public function home_ddxx_gcz_cl()
    {
        ...
        if ($ppddxx['g_user'] <> $_SESSION['uname']) {
            ...
            if ($data_P['comfir'] == '1') {
                ...
                M('ppdd')->where(array('id' => $data_P['id'], 'zt' => '1'))
                         ->save(array('zt' => '2', 'qr_zt' => 2, 'dk_date' => date('Y-m-d H:i:s', time())));//更新此订单状态
                //获取接收帮助交易的金额
...
// 			    }
                ...
                //此处给wanjiali强行增加 逻辑  给充值人钱包增加400的金币===add by QQ7344662 start
                //  $yuanlai_jb=$tgbz_user_xx['ue_money'];
                //if($czyqr==300){
                //	$xin_jb=$yuanlai_jb+$czyqr+400;
                //}else{
                //	$xin_jb=$yuanlai_jb+$czyqr+800;
                //}
                //$xin_jb=$yuanlai_jb+150;
                //	$jiangli=$yuanlai_jb*0.35;
                M('user')->where(array('UE_ID'=>$tgbz_user_xx['ue_id']))->setInc('UE_money',$ppddxx['jb']);
                //此处给wanjiali强行增加 逻辑  给充值人钱包增加400的金币===add by QQ7344662 end
                ...
                if ($ppddxx['ts_zt'] == '2') {
                    ...
                    if ($data_P['face180'] == '') {
                        ...
                        M('ppdd')->where(array('id' => $data_P['id'], 'zt' => '1'))
                                 ->save(array('ts_zt' => '2', 'pic2' => $data_P['face180']));
...
// echo "Tomorrow:",date('Y-m-d H:i:s',$aab2);die;
    ...
    {
        if (IS_POST) {
            $paidan_zs = M('paidan')->where(array('user' => $_SESSION['uname'], 'zt' => 0))->count();
            $data_P    = I('post.');
            //$user = M ( 'user' )->where ( array (UE_account => $_SESSION ['uname']) )->find ();
            //$user1 = M ();
            if (empty($data_P['user'])) {
            ...
            if ($userxx['ue_secpwd'] <> md5($data_P['ejmm'])) {
                ...
                if (!preg_match('/^[0-9]{1,10}$/', $data_P ['sh']) || !$data_P ['sh'] > 0) {
                    ...
                    $pai_zs_df = M('paidan')->where(array('user' => $data_P['user'], 'zt' => 0))->count();
                    ...
                    for ($i = 0; $i < $data_P['sh']; $i++) {
                        $pai_xx = M('paidan')->where(array('user' => $_SESSION['uname'], 'zt' => 0))->find();
                        M('paidan')->where(array('id' => $pai_xx['id'], 'zt' => 0))
                                   ->save(array('user' => $data_P['user']));
                    }

                    $pai_zs_xz    = M('paidan')->where(array('user' => $_SESSION['uname'], 'zt' => 0))->count();
                    $pai_zs_df_xz = M('paidan')->where(array('user' => $data_P['user'], 'zt' => 0))->count();

                    $note3                   = "排单币转出给" . $data_P['user'];
                    $record3 ["UG_account"]  = $_SESSION['uname']; // 登入转出账户
                    $record3 ["UG_type"]     = 'mp';
                    $record3 ["UG_allGet"]   = $paidan_zs; // 金币
                    $record3 ["UG_money"]    = '-' . $jbhe; //
                    $record3 ["UG_balance"]  = $pai_zs_xz; // 当前推荐人的金币馀额
                    ...
                    $record3 ["UG_account"]  = $data_P['user']; // 登入转出账户
                    $record3 ["UG_type"]     = 'mp';
                    $record3 ["UG_allGet"]   = $pai_zs_df; // 金币
                    $record3 ["UG_money"]    = '+' . $jbhe; //
                    $record3 ["UG_balance"]  = $pai_zs_df_xz; // 当前推荐人的金币馀额
    ...
    public function jihuo()
    {
        if (IS_POST) {
            ...
            if ($userxx['ue_check'] == 1) {
                ...
                if (!empty($pin_data)) {
                    M('pin')->where(array('pin' => $data_P ['sh']))
                            ->save(array('zt' => '1', 'sy_user' => $data_P [''], 'sy_date' => date('Y-m-d H:i:s', time())));
                    M('user')->where(array('UE_account' => $data_P['user']))
                             ->save(array('UE_check' => 1, 'pin' => $data_P['sh']));
                } else {
    ...
    public function can_tgbz()
    {
        ...
        echo json_encode(array("result" => $flag, "msg" => $msg));
