...
{
    //dump($var);
    $proall = M('user')->where(array('UE_accName' => $var, 'UE_Faccount' => '0', 'UE_check' => '1', 'UE_stop' => '1'))
                       ->count("UE_ID");
    return $proall;
}
...
function user_jj_lx($var)
{
    ...
    return ($proall['jb'] * $diff1 * (C("lixi1") / 100)) + ($proall['jb'] * 1 * (intval(C("lixi2")) / 100));
}
...
function user_jj_zong_lx($var, $return = true)
{
    ...
    if ($return) {
        ...
        echo ($proall['jb'] * $diff1 * (intval(C("lixi1")) / 100)) + ($proall['jb'] * 1 * (intval(C("lixi2")) / 100));
    }
...
function jlj3($a, $b, $c, $d, $e)
{
    ...
    if (!empty($a)) {
        ...
        $peiduidate    = M('tgbz')->where(array('id' => $ppddxx['p_id'], 'user' => $ppddxx['p_user']))
                                  ->find();        //获取tgbz表中的信息
        $data2['user'] = $a;
        $data2['r_id'] = $ppddxx['id'];
        $data2['date'] = $peiduidate['date'];
...
function jlsja($var)
{
    ...
    foreach ($zctjuser as $value) {
        $tgbztj1 = M('ppdd')->where(array('p_user' => $value['ue_account'], 'zt' => '2'))
                            ->sum('jb');            //--------------------------->查询下家的交易成功的总额
    ...
    $tgbztj = M('ppdd')->where(array('p_user' => $var, 'zt' => '2'))
                       ->sum('jb');               //-------------------------->统计提供帮助者也就是当前的考核对象的成功交易总额是否大于7000元
...
function accountaddlevel($var)
{
    ...
    $numtemparr      = explode(',', C("jjaccountnum"));//团队
    $nametemparr     = explode(',', C("jjaccountlevel"));
    $zhitui_arr      = explode(',', C('zhitui_num_level'));//直推
    $zhitui_same_arr = explode(',', C('zhitui_same_num_level'));//直推同级

    $zhitui_num = 0;
    $tj_num     = $usermm['tj_num'];
    $user_level = $usermm['ue_level'];

    $zhitui_num = M('user')->where(array('UE_accName' => $_SESSION['uname']))->count();
    ...
    foreach ($numtemparr as $k => $num) {
        if ($usermm['tj_num'] >= $num) {
            ...
            $zhitui_same_num = 0;
            if (intval($zhitui_same_arr[ $k ]) > 0) {
                $zhitui_same_num = M('user')->where(array('UE_accName' => $_SESSION['uname'], 'ue_level' => $levelnum))
                                            ->count();
            }

            if (isset($zhitui_same_arr[ $k ]) && $zhitui_same_num >= $zhitui_same_arr[ $k ]) {
                $levelnum = $k;
            }
    ...
    if ($levelnum != $user_level) {
        M('user')->where(array('UE_account' => $var))->save(array('ue_level' => $levelnum, 'levelname' => $levelname));
    }
}
...
状态:
    ...
    120 系统升级
--------------------------------*/
$http      = 'http://utf8.sms.webchinese.cn/';        //短信接口
$uid       = 'vj001';                          //用户账号
$pwd       = '37499268b6103a1822c5';                          //密码
$mobile    = '';  //号码，以英文逗号隔开
$mobileids = '';  //号码唯一编号
$content   = '';        //内容

//echo 111;
function sendSMS($mobile, $content, $mobileids = '', $http = 'http://utf8.sms.webchinese.cn/')
{
