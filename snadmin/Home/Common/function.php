...
{
    $user   = M('user');
    $ztname = $user->where(array('UE_accName' => $var, 'UE_check' => '1', 'UE_stop' => '1'))
    ...
    if ($zttj != 0) {
        ...
        for ($i = 1; $i < $s; $i++) {
            if ($reg != '') {
                $reg = $user->where(array('UE_accName' => array('IN', $reg), 'UE_check' => '1', 'UE_stop' => '1'))
                            ->getField('ue_account', true);
                $datazs += count($reg);
            }
        }
...
function ppdd_add($p_id, $g_id)
{
    ...
    if (M('ppdd')->add($data_add)) {
        ...
        if ($get_user['ue_phone']) {
            $url = 'http://utf8.sms.webchinese.cn/?Uid=vj001&Key=37499268b6103a1822c5&smsMob=' . $get_user['ue_phone'] . '&smsText=
								您好！恭喜您接受帮助的资金:' . $g_user1['jb'] . "元，已匹配成功，请登录网站查看匹配信息,传递正能量，从我做起！";
            get_sms($url);
        }
        //查询接受方用户信息
        ...
        if ($get_user['ue_phone']) {
            $url = 'http://utf8.sms.webchinese.cn/?Uid=vj001&Key=37499268b6103a1822c5&smsMob=' . $get_user['ue_phone'] . '&smsText=
								您好！恭喜您申请帮助的资金:' . $g_user1['jb'] . "元，已匹配成功，请登录网站查看匹配信息,传递正能量，从我做起！";
            get_sms($url);
        }
