...
class IndexController extends CommonController
{
    ...
    public function shaoshang(){
    	...
    	$list = $User->where(array('UE_level'=>'1'))->select();
    	
    	foreach ($list as $user){
    		$name=$user['ue_account'];
    		$tgbz=M('tgbz');
    		$bz_data=$tgbz->where(array('user'=>$name))->order('date')->limit(1)->find();
    		$date=strtotime('+2 d',strtotime($bz_data['date']));
    		if(time()>=$date){
    			$User_1 = M('user');
    			$User_1->where(array("UE_ID"=>$user['ue_id']))->save(array('jl_he'=>0,'tj_he'=>0));
    		}
    	}
...
//             foreach ($list as $k=>$v)
//             {
//                 $list[$k]['levelname'] = $nametemparr[$v['ue_level']];
//             }            
//         }
   ...
   /* array (
    ...
    public function jjset()
    {
        if (IS_POST) {
            ...
            //早于排单开始时间提示语
            $this->paidan_time_start_tips = C("paidan_time_start_tips");

            //晚于排单结束时间提示语
            $this->paidan_time_end_tips = C("paidan_time_end_tips");
            ...
            //会员升级直推同级别人数
            $this->zhitui_same_num_level =C('zhitui_same_num_level');
            
            //动态收益奖金比率
            $this->tgbz_min_month =C('tgbz_min_month');
            
            //动态收益奖金无限代比率
            $this->jj_wx_rate =C('jj_wx_rate');            
            
            //动态收益奖金可获得代数
            $this->jj_daishu =C('jj_daishu');
    ...
    public function txset()
    {
        if (IS_POST) {
            ...
            $this->tj_beishu = C("tj_beishu");
			 ...
			 $this->home_show_m = C("home_show_m");
            $this->home_show_p = C("home_show_p");
